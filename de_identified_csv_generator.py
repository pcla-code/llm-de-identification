...
# Specify the relative folder paths for ground truth and original files
gt_coded_folder = 'Ground truth files/'
original_folder = 'original files/'
output_folder = 'OpenAI_coded_files/'

# Set the API key for openai
openai.api_key = 'Enter api key here'  # Make sure to use your own API key
...
for gt_file in gt_files:
    if gt_file.endswith('.csv'):
        try:
            ...
            gt_coded = pd.read_csv(os.path.join(gt_coded_folder_path, gt_file))
            original = pd.read_csv(os.path.join(original_folder_path, original_file))
            ...
            if not common_ids.empty:
                ...
                new_df = merged[['post_text_original', 'post_text_coded']].copy()
                ...
                for index, row in prompt_df.iterrows():
                    prompt = row['prompt']
                    prompt_df_temp = new_df.copy()  # Create a copy of the DataFrame for the current prompt
                    ...
                    # Save the CSV for the current prompt
